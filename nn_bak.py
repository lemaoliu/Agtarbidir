    ...
    try:
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                '''
                tbc = f_bi_cost(*f_inputs)
                c = enc_dec.f_cost(x,x_mask,y,y_mask)
                rc = rev_enc_dec.f_cost(r_x,r_x_m,r_y,r_y_m)
                print 'bi_cost=%f, cost=%f, rev_cost=%f'%(tbc,c,rc)
                print "x", x
                print "x_mask", x_mask
                print "y", y
                print "y_mask", y_mask
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    if (uidx == 0 or
                        valid_err <= numpy.array(history_errs)[:,
                                                               0].min()):
                        best_p = enc_dec.unzip() #unzip(tparams)
                        bad_counter = 0

                    print 'gold', y[:,0]
                    print ('Train ', train_err, 'Valid ', valid_err,
                           'Test ', test_err)#, 'Train_cost', train_cost)
            ...
            if dev_file and eidx > begin_valid:
                ...
                test_eval_combine(beamsearch,beamsearch_rev,src_file=dev_file,\
                                  trg_file=dev_ref,src_xml=dev_xml)
                if 1 and tst_file:
                    test_eval_combine(beamsearch,beamsearch_rev,src_file=tst_file,\
                              trg_file=tst_ref,src_xml=tst_xml,isdev=False)
...
def test_eval_combine(
    ...
    for i, s in enumerate(open(src_file)):
        best_tran,best_f, best_r = combine(beamsearch,beamsearch_rev,s)
        if (i + 1)  % 100 == 0:
            ff.flush()
    ...
    print 'ACC for combination',dev_or_tst
    evaluation(
        res=res,
        src_xml=src_xml,
        trg_file=trg_file,
...
def combine(
        ...
        for y in trans2:
            ...
            for px in xrange(len(x)+1):
                for py in xrange(len(y)+1):
                    ...
                    #    cost1_r = beamsearch_rev.prob_bisent_indice(s,t1)
                    #    cost1_f = beamsearch.prob_bisent_indice(s,t1)
                    #    heap.append((-cost1_f-cost1_r,t1,cost1_f,cost1_r, x, px, y, py,'xy'))
                        trans_set.add(tt1)
                        cands.append(t1)
                    #    cost_f_seq.append(cost1_f)
                    #    cost_r_seq.append(cost1_r)
                    if 0 and tt2 not in trans_set:
                    #    cost2_f = beamsearch.prob_bisent_indice(s,t2)
                    #    cost2_r = beamsearch_rev.prob_bisent_indice(s,t2)
                    #    heap.append((-cost2_f-cost2_r,t2,cost2_f,cost2_r, x, px, y, py, 'yx'))
                        trans_set.add(tt2)
                        cands.append(t2)
                    #    cost_f_seq.append(cost2_f)
                    #    cost_r_seq.append(cost2_r)
    e_time = time.time()
    #logger.debug("Current calculating prob speed is {} per sentence for {} sentences".
    #    format((time.time() - d_time) / len(heap),len(heap)))
    ...
    heap = [(score,tran) for score,tran in zip(costs,cands)]
    #logger.debug("Current calculating prob speed is {} per sentence for {} sentences batch_time".
    #    format((time.time() - e_time) / len(heap),len(heap)))

    kbest = heapq.nlargest(beamsearch.beamsize, heap)

    #print src_sent, kbest
    return beamsearch.to_words(kbest[0][1],beamsearch.t_index2word),best_f,best_r
...
def evaluation(
    ...
    evaluator = "/panfs/panltg2/users/finch/expt/NEWS2012/scripts/news_evaluation.py"
...
def test_eval(
    ...
    if beamsearch is None:
        config = get_options(model)
        enc_dec = EncoderDecoder(config)
        enc_dec.load(model)
        srcdict,srcdict_rev = load_dict(src_dict,True)
        trgdict,trgdict_rev = load_dict(trg_dict,True)
        enc_dec.BuildEncDec()
    ...
    start_time = time.time()
    for i, s in enumerate(open(src_file)):
        _, best_tran = beamsearch.decode(s)
        if (i + 1)  % 100 == 0:
            ff.flush()
            logger.debug("Current speed is {} per sentence".
                    format((time.time() - start_time) / (i + 1)))
